...
from utils import print_and_add_to_log, get_datasets_for_ViT, \
    Identity, get_finetuned_features, extract_fetures
...
if __name__ == '__main__':
    ...
    for _class in _classes:
        ...
        for filter in range(3):
            ...
            print_and_add_to_log(f"filter is : {filter}", logging)
            print_and_add_to_log("===================================", logging)
            args['_class'] = _class
            ...
            if not os.path.exists(join(model_path, 'best_full_finetuned_model_state_dict.pkl')):
                continue
            ...
            if filter == 0:
                ...
                with open(join(BASE_PATH, f'{"unimodal" if args["unimodal"] else "multimodal"}',
                            f'{args["dataset"]}/class_{args["_class"]}/extracted_features',
                            'test_pretrained_ViT_features_' + str(filter) + '.npy'), 'rb') as f:
                    test_features = np.load(f)
            ...
            FINETUNED_PREDICTION_FILE_NAME = 'full_test_finetuned_scores_' + str(filter) + '.npy'
