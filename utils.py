...
from filtered_dataset import CIFAR10_filtered, CIFAR100_filtered
...
def extract_fetures(base_path,
    ...
    for dataset in datasets:
        ...
        if manual_class_num_range is not None:
            ...
            _classes = manual_class_num_range
        ...
        for _class in _classes:
            ...
            for unimodal in unimodal_vals:
                ...
                # get dataset
                trainset_origin, testset = get_datasets(dataset, data_path, val_transforms, filter=filter)
                ...
                if calculate_features or not os.path.exists(
                        join(extracted_features_path, 'test_pretrained_ViT_features_' + str(filter) + '.npy')):
...
def get_datasets(dataset, data_path, val_transforms, filter=0):
    if dataset == 'cifar100':
        testset = CIFAR100_filtered(root=data_path,
        ...
        testset = CIFAR10_filtered(root=data_path,
                          train=False, download=True,
                          transform=val_transforms,
                          filter=filter)
...
def plot_accuracy(train_accuracies, val_accuracies, to_show=False,
